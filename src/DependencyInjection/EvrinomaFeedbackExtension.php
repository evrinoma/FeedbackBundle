...
class EvrinomaFeedbackExtension extends Extension
{
    use HelperTrait;

    public const ENTITY = 'Evrinoma\FeedbackBundle\Entity';
    public const MODEL = 'Evrinoma\FeedbackBundle\Model';
    public const ENTITY_FACTORY_FEEDBACK = FeedbackFactory::class;
    public const ENTITY_BASE_FEEDBACK = BaseFeedback::class;
    public const DTO_BASE_FEEDBACK = FeedbackApiDto::class;
    ...
    public function load(array $configs, ContainerBuilder $container)
    {
        ...
        if (self::ENTITY_FACTORY_FEEDBACK !== $config['factory']) {
    ...
    private function wireConstraintTag(ContainerBuilder $container): void
    {
        foreach ($container->getDefinitions() as $key => $definition) {
            switch (true) {
                case str_contains($key, FeedbackPass::FEEDBACK_CONSTRAINT)   :
                    $definition->addTag(FeedbackPass::FEEDBACK_CONSTRAINT);
                    break;
                default:
            }
        }
    ...
    private function wireRepository(ContainerBuilder $container, Reference $registry, string $madiator, string $class, string $driver): void
    {
        ...
        $container->addAliases([FeedbackCommandRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
        $container->addAliases([FeedbackQueryRepositoryInterface::class => 'evrinoma.'.$this->getAlias().'.repository']);
    }
    ...
    {
        return EvrinomaFeedbackBundle::BUNDLE;
    }
}
